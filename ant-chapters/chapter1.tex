...
\begin{pr}[Relationship with roots of minimal polynomial]\llabel{ntr-min-roots}
...
This is a nice property that connects minimal polynomials of $\be$ to the properties of $m_\be$. It also lets us compute norm and trace of $\be$ basically immediately as long as we know its Galois conjugates.
...
\begin{rem}[Norm depends on the field we work in]\llabel{norm-depends-on-extension}
...
The second proposition is related to the integrality of trace and norm and follows directly from our previous results:

\begin{pr}[Integrality]\llabel{ntr-integrality}
%Let $A$ be an integral domain with fraction field $K$, let $L$ be a finite %separable (not nec.)
...
\begin{pr}[Relationship with embeddings]\llabel{ntr-embeddings}
...
Let $\be_1,\ldots, \be_m$ be the conjugates of $\be$. There are $m$ distinct embeddings $K(\be)\to M$; they each take $\be$ to a different $\be_k$. Each of these embeddings extend to $r:=[L:K(\be)]=\frac{n}{m}$ embeddings $L\to M$. %\footnote{If $K\subeq K'\subeq L\subeq M$ and $M$ is Galois over $K$, then each embedding $K'\to M$ fixing $K$ extends to $[L:K']$ embeddings $L\to M$ fixing $K$. Proof: Let $\pi$ be the quotient map $G(M/K)/G(M/L)\to G(M/K)/G(M/K')$. Then $\ker \pi=[L:K']$.}
Now use Proposition~\ref{ntr-min-roots}.
\end{proof}

\begin{pr}[Transitivity of trace and norm]\llabel{ntr-transitivity}
...
\begin{exr}[Check your understanding]$\,$
\vspace{0cm}
\begin{enumerate}
